...
`issue-onboard`가 live issue 목록을 읽는 동안 또는 최종 추천 출력 직전에 `.issue/graph.json`이 바뀌어도, 이전에 메모리에 적재한 정상 그래프로 추천을 계속하던 P1 결함을 수정했습니다.

live snapshot 직후와 최종 추천 직전에 그래프 캐시를 다시 읽고 검증하며, 캐시가 깨졌거나 동기화가 실패하면 `ONBOARD_COUNT`와 `PRIORITY`를 출력하지 않고 종료합니다.
...
| 최종 그래프 로드 뒤 추천 직전에 그래프를 `{ TOCTOU invalid`로 변경 | 해당 경계가 없어 추천 진행 | `CLI_EXIT=1`, 추천 출력 없음 |
...
최종 그래프 로드 뒤 캐시 변경 검증: `.issue/19/evidence/after/final-cache-validation.txt` (`EXIT=1`, `RECOMMENDATION=none`)

CLI/백엔드 검증 경계의 결함이라 화면 캡처는 생략했습니다. 동일한 fixture와 명령을 사용해 상태·출력·종료 코드를 비교했습니다.

## 변경 파일

- `skills/issue-onboard/scripts/issue-onboard.mjs` — live snapshot 이후와 최종 추천 출력 직전 그래프 캐시 재검증
- `skills/issue-onboard/scripts/bootstrap.test.mjs` — 동시 캐시 변경, invalid cache, 최종 로드 경계 회귀 테스트

## 검증

- `node --test skills/issue-onboard/scripts/*.test.mjs` — 46 passed
...
- `node --test --test-name-pattern='reports invalid caches and fails when bootstrap fails|onboarding CLI bootstraps a complete sync|graph cache changes after the final graph load' skills/issue-onboard/scripts/bootstrap.test.mjs` — 3 passed
