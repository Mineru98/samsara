...
최종 검증 기준 source commit: `61471f3fbbddb69801fff8c43c4d7c6917b62a51`
...
모든 공식 graph writer는 `.issue` 디렉터리와 `graph.json` 심볼릭 링크도 거부해 저장소 밖 파일을 변경하지 않습니다.
...
| 여섯 공식 `patchGraphNode`의 `.issue`/`graph.json` 링크 입력 | 모두 `false`, 외부 graph 불변 |
...
- `node --test skills/issue-onboard/scripts/*.test.mjs` — 50/50 passed
- `node --test tools/issue-ontology/ontology.test.mjs` — 13/13 passed
- 변경 JavaScript 8개 `node --check` — 모두 통과
- targeted cache/lock/symlink 회귀 테스트 — 7/7 passed
