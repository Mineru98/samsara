...
각인은 순환에서 갈라져 나올 뿐 **순환을 끊지 않습니다.** 인은 각인 뒤에도 계속 맺히고 풀리며, 다음 각인은 지난 각인 이후에 쌓인 커밋만을 비문에 새깁니다.
...
>
> **[술식]** `major`·`minor`·`patch` 중 하나를 받아 현재 버전을 판정하고 한 단계 올립니다. 버전이 박힌 8개 파일을 함께 갱신한 bump PR 을 만들고 거기서 멈추며, 그 PR 이 merge 된 뒤 다시 부르면 태그를 달고 직전 태그 이후의 커밋을 type 별로 묶은 릴리즈를 발행합니다. 태그도 파일에 적힌 버전도 없는 첫 릴리즈라면 인자와 무관하게 `v0.1.0` 에서 시작합니다.

```bash
# 지금 태그가 v1.4.2 라면
/issue-version patch     # → v1.4.3   버그 수정·문서 정리
/issue-version minor     # → v1.5.0   기능 추가
/issue-version major     # → v2.0.0   호환성이 깨지는 변경
```

**한 번 불러서는 절반까지만 갑니다.** 1단계가 만든 bump PR 을 사람이 검토해 merge 한 뒤, 같은 명령을 한 번 더 불러야 2단계에서 태그가 박힙니다.
