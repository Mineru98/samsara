...
import {
  mkdirSync, existsSync, readFileSync, writeFileSync, writeSync, cpSync, readdirSync, rmSync, realpathSync, lstatSync, unlinkSync, renameSync, openSync, closeSync, fstatSync, constants,
} from 'node:fs';
 ...
 * 설치 형태는 플러그인 루트(`<repo>/skills/`), 프로젝트 로컬
 * (`<repo>/.claude/skills/`), 홈 전역(`~/.claude/skills/`), 그리고
 * 저장소를 홈 아래로 링크한 개발 설치다.
 *
 * 예전에는 프로젝트 로컬만 뒤졌다. 그래서 홈 전역으로만 깐 환경에서는
 * issue-onboard ↔ issue-sync 가 서로를 못 찾고 죽었고,
...
export function resolveSkillScript(selfUrl, skill, script, { root } = {}) {
  ...
  if (root) add(path.join(root, 'skills'));
...
export function requireSkillScript(selfUrl, skill, script, opts = {}) {
  ...
  if (file) return file;
  fail(
    `${skill} 스킬을 찾지 못했다. 플러그인의 skills/ 또는 사용자 스킬 디렉터리에 `
...
function acquireGraphCacheLock(root) {
  ...
  try {
    ...
    try {
      ...
      if (retryError.code === 'EEXIST') throw new Error('그래프 캐시가 다른 작업에서 변경 중이라 갱신하지 않는다.');
      throw retryError;
    }
  }
  let released = false;
