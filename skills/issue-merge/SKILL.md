...
<skill>
  ...
  <subagents>
    <agent name="shikamaru-merge-analyst" claude-model="haiku" codex-model="gpt-5.6-luna">
      워크트리 하나씩 분석. 워크트리 개수만큼 병렬로 스폰
    </agent>
    <agent name="yamato-merge-resolver" claude-model="sonnet" codex-model="gpt-5.6-terra">
      충돌 헌크를 양쪽 의도를 보존하는 방향으로 해소. 충돌이 있는 워크트리마다 스폰
    </agent>
    <agent name="itachi-merge-critic" claude-model="haiku" codex-model="gpt-5.6-luna">
  ...
  <reporting>
    ...
    마무리 요약과 완료 브리핑은 `references/ask.md` 6절의 **항목/내용 2열 마크다운 표**로 낸다. `text` 정렬 블록이 아니다.
    질문·문제 보고는 3·4절의 `text` 블록을 그대로 쓴다.
...
자동 오케스트레이터는 `node <skill>/scripts/issue-merge.mjs phase --request
<absolute-canonical-json>`만 사용한다. 요청 스키마는
`contracts/issue-merge-phase-api-v1.json`, 전체 9단계 능력·효과·승인·체크포인트·
재조정 계약과 raw-byte 폐쇄는 `contracts/issue-phase-capability-bundle-v1.json`이다.
누적 preflight, PR별 승인, 통합 검증 뒤 close, 마지막 cleanup 순서는 유지하며,
계약 또는 폐쇄 검증이 실패하면 active 실행을 시작하지 않는다.
...
```text
skills/issue-merge              # 플러그인 루트 (Claude Code / Codex)
~/.claude/skills/issue-merge    # 홈 설치
~/.codex/skills/issue-merge     # 홈 설치
```

플러그인으로 실행할 때는 호스트가 실행 계열을 결정한다. 홈 설치에서는 `<skill>` 이
`.claude/` 밑이면 **claude**, `.codex/` 밑이면 **codex** 다.

# 서브에이전트

```text
claude  agents/shikamaru-merge-analyst.md   (model: haiku)
        agents/yamato-merge-resolver.md     (model: sonnet)
        agents/itachi-merge-critic.md       (model: haiku)
codex   agents/shikamaru-merge-analyst.toml (model = "gpt-5.6-luna")
        agents/yamato-merge-resolver.toml   (model = "gpt-5.6-terra")
        agents/itachi-merge-critic.toml     (model = "gpt-5.6-luna")
```
...
```bash
sh <migrate-skill-agent>/scripts/migrate-skill-agent.sh --agent shikamaru-merge-analyst --target home --link --clone
sh <migrate-skill-agent>/scripts/migrate-skill-agent.sh --agent yamato-merge-resolver  --target home --link --clone
sh <migrate-skill-agent>/scripts/migrate-skill-agent.sh --agent itachi-merge-critic    --target home --link --clone
```
...
**4지선다를 묻기 직전에, 방금 끝낸 통합의 전모를 `references/ask.md` 6절의 완료 브리핑 표로 먼저 렌더링한다.**
표 값은 새로 조회하지 않고 `inventory` 출력과 merge · close · 통합 테스트 결과를 그대로 쓴다. 브리핑표는 선택지를 대체하지 않는다 — 표를 낸 뒤 이어서 `references/next-actions.md` 의 4지선다를 그대로 제시한다.

## 마무리 보고

`references/ask.md` 6절의 표 골격을 그대로 쓴다.
