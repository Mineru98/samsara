...
선택지를 묻기 전에, 방금 끝낸 통합의 전모를 **항목/내용 2열 마크다운 표**로 한 번 보여준다.
사용자는 다음 행동을 고르기 전에 무엇이 merge·보류·close 됐고 통합 테스트가 어땠는지부터 확인한다.
표 골격과 항목은 `references/ask.md` 6절을 그대로 따른다(대상 / 충돌 / merge 됨 / 보류 / 통합 테스트 / close 됨 / 정리 / 남은 것 / 다음).
값은 새로 조회하지 않고 `inventory` 출력과 이번 회차의 merge·close·통합 테스트 결과를 그대로 쓴다.
이 표는 브리핑이며 **아래 선택지를 대체하지 않는다.** 표를 낸 다음 곧바로 선택지를 묻는다.
...
후보가 없으면 3번을 빼고 남은 선택지를 위로 당겨 다시 번호를 매긴다.

## 후속 이슈 후보란

통합·재검증을 돌리며 손대지 못하고 남긴, 지금 통합 범위 밖의 별도 작업이다.

- 통합 테스트에서 드러난 회귀 — 어떤 조합에서 무엇이 깨졌는지가 이미 손에 있다
- 통합 중 발견했지만 이번 회차에서 미룬 개선점·정리
...
- **통합 테스트에서 회귀가 나왔으면** 회귀는 그 자체로 후속 이슈 후보다. 3번(새 이슈 등록)을 넣고 권장으로 올린다. 회귀는 기억이 아니라 이슈로 남겨야 한다.
- 남은 열린 이슈가 하나도 없으면 1번(다른 이슈 착수)을 선택지에서 뺀다. 이때 후속 이슈 후보가 있으면 3번(새 이슈 등록)을 권장으로 올리고, 없으면 2번 또는 4번이 권장이 된다.
- 그 외에는 1번이 권장이다.
