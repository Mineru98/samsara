...
**워크트리 개수만큼** `shikamaru-merge-analyst` 를 병렬로 띄운다. 각 에이전트에는 하나만 배정한다.
...
충돌이 있는 워크트리마다 `yamato-merge-resolver` 를 띄운다. 여러 개면 병렬.
...
`plan.md` 를 `itachi-merge-critic` 에 넘긴다. **이 단계를 건너뛰지 않는다.**
