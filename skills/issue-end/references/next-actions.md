...
3번은 고정 선택지가 아니다. **후속 이슈 후보가 있을 때만** 넣는다. 판정은 아래 `후속 이슈 후보란` 을 따른다.
후보가 없으면 3번을 빼고 남은 선택지를 위로 당겨 다시 번호를 매긴다(종료가 3번이 된다).

## 후속 이슈 후보란

작업하면서 손대지 못하고 남긴, 지금 이슈 범위 밖의 별도 작업이다.

- 구현·검증 중 발견한 다른 버그나 회귀
- 범위를 벗어나 이번에 미룬 개선점·리팩터링·TODO
- 사용자가 대화 중 꺼낸 또 다른 변경 요청
...
- 남은 열린 이슈가 하나도 없으면 1번(다른 이슈 착수)을 선택지에서 뺀다. 이때 후속 이슈 후보가 있으면 새 이슈 등록을, 없으면 워크트리 merge 를 권장으로 올린다.
- 후속 이슈 후보(회귀·미룬 개선점·사용자가 더 말한 건)가 있으면 새 이슈 등록을 넣고, 회귀처럼 반드시 남겨야 하는 후보면 권장으로 올린다.
