# 다음 행동

PR 까지 끝났으면 사용자가 다음에 무엇을 할지 고르게 한다. 여기서 흐름이 끊기면 워크트리만 쌓인다.

## 완료 브리핑 (선택지 앞에 먼저 낸다)

선택지를 묻기 전에, 방금 마친 작업의 전모를 **항목/내용 2열 요약표**로 한 번 보여준다.
사용자는 다음 행동을 고르기 전에 무엇이·어디서·어떤 증거로·어느 PR·상태로 끝났는지부터 확인한다.
이 표는 브리핑이며 **아래 AskUserQuestion 을 대체하지 않는다.** 표를 낸 다음 곧바로 선택지를 묻는다.

값은 새로 조회하지 않는다. 이미 손에 있는 출력을 그대로 쓴다 — `context` 출력(`issue`/`issueTitle`/
`branch`/`baseBranch`/`worktrees[].display`/`openPr`/`evidence`/`isPrivate`), `urls` 출력의 코멘트 URL,
현재 status 라벨, 그리고 이번 실행에서 만든 PR 번호·URL. 링크·경로는 SKILL.md 의
`링크와 경로 쓰는 법` 을 따른다(이슈·PR·코멘트는 `[설명](링크)`, 워크트리는 배치에 맞는 절대/상대 경로).

| 항목 | 내용 |
| --- | --- |
| 이슈 | `[#{issue} {issueTitle}]({issueUrl})` |
| 브랜치 | 작업 브랜치 이름 (예: `feat/3-issue-3`) |
| 워크트리 | `worktrees[].display` + 배치 (예: `/Users/me/work/repo-issue-3` (sibling)) |
| 기본 브랜치 | `baseBranch` (판별 출처, 예: `main` (origin/HEAD)) |
| 증거 | 증거 종류 요약 (예: `문서성 변경 — 전후 diff 텍스트 (webp 이미지 없음)`, 또는 `before 2장 / after 2장 (박스 2개)`) |
| 코멘트 | `[리포트 보기]({코멘트 URL})` + 신규 / 기존 갱신 / 갱신 불필요 |
| PR | `[#{번호} {제목}]({PR URL})`, 또는 `만들지 않음 — 사유` |
| 상태 | 현재 status 라벨 (예: `status:review`) |
| 후속 후보 | 작업 중 발견한 별도 이슈 후보 한 줄 (없으면 `없음`) |

- **값이 없으면 빈칸으로 두지 않는다.** 사유를 적는다 (예: 증거 이미지 없음 → 변경 성격, PR 없음 → 사유).
- **후속 후보** 행은 아래 `후속 이슈 후보란` 판정과 같은 대상을 쓴다. 있으면 그 후보가 선택지 3번(새 이슈 등록)의 근거가 된다.
- 저장소를 식별하지 못해 URL 을 만들 수 없으면 번호만 적고 그 사실을 한 줄 남긴다. 없는 링크를 지어내지 않는다.

## 선택지 (순서 고정)

AskUserQuestion 으로 제시한다. 순서를 바꾸지 않는다. 새 작업을 이어가는 쪽이 기본값이다.
질문 본문에 현재 단계를 함께 적는다 — 여기가 이 스킬의 마지막 단계라는 사실 자체가 사용자에게 필요한 정보다.

```text
질문   issue-end 11단계(다음 행동 선택)입니다. PR 까지 끝났습니다. 다음으로 무엇을 할까요?

1. 다른 이슈 착수 (권장)   등록된 열린 이슈 중 하나를 골라 $issue-start
2. 워크트리 전부 merge     지금까지 쌓인 워크트리를 모아 $issue-merge
3. 새 이슈 등록            $issue-create   ← 후속 이슈 후보가 있을 때만 넣는다
4. 종료                    여기서 마친다
```

3번은 고정 선택지가 아니다. **후속 이슈 후보가 있을 때만** 넣는다. 판정은 아래 `후속 이슈 후보란` 을 따른다.
후보가 없으면 3번을 빼고 남은 선택지를 위로 당겨 다시 번호를 매긴다(종료가 3번이 된다).

## 후속 이슈 후보란

작업하면서 손대지 못하고 남긴, 지금 이슈 범위 밖의 별도 작업이다.

- 구현·검증 중 발견한 다른 버그나 회귀
- 범위를 벗어나 이번에 미룬 개선점·리팩터링·TODO
- 사용자가 대화 중 꺼낸 또 다른 변경 요청

이런 후보가 하나도 없으면 "새 이슈 등록" 은 물어볼 것이 없는 질문이다. 등록할 내용이 없는데 매번 등록을 권하지 않는다.
후보가 있으면 무엇을 등록할지 한 줄로 함께 제시한다.

선택지를 제시하기 전에 재료를 모아 함께 보여준다.

```bash
gh issue list --state open --limit 10 --json number,title,url,labels
git worktree list
```

- 남은 열린 이슈가 하나도 없으면 1번(다른 이슈 착수)을 선택지에서 뺀다. 이때 후속 이슈 후보가 있으면 새 이슈 등록을, 없으면 워크트리 merge 를 권장으로 올린다.
- 후속 이슈 후보(회귀·미룬 개선점·사용자가 더 말한 건)가 있으면 새 이슈 등록을 넣고, 회귀처럼 반드시 남겨야 하는 후보면 권장으로 올린다.
- 워크트리가 이 작업 하나뿐이면 2번 설명에 "현재 1개"라고 적는다.

이슈·PR 은 `[설명](링크)` 형식으로, 워크트리 경로는 `context` 출력의 `worktrees[].display` 값으로 보여준다. 세부는 SKILL.md 의 `링크와 경로 쓰는 법`.

## 1번 — 다른 이슈 착수

이슈 목록을 번호·제목·라벨로 보여주고 고르게 한 뒤 `$issue-start <번호>` 로 넘긴다.

현재 워크트리에서 그대로 시작하지 않는다. `issue-start` 가 새 워크트리를 만든다.

## 2번 — merge

`issue-merge` 로 위임한다. **이 스킬이 직접 merge 하지 않는다.**

```text
$issue-merge
```

넘길 때 아래를 함께 전달한다.

- 방금 만든 PR 번호와 이슈 번호
- `git worktree list` 결과
- 기본 브랜치 이름

`issue-merge` 가 없으면 설치한다.

```bash
sh <migrate-skill-agent>/scripts/migrate-skill-agent.sh \
  --skill issue-merge --target home --link --clone --flavor <claude|codex>
```

설치도 안 되면 merge 를 대신 수행하지 말고, 사용자에게 직접 `gh pr merge` 를 실행하라고 안내한다. 통합 검증 절차 없이 merge 하는 것이 이 스킬군이 막으려는 상황이다.

## 3번 — 새 이슈 등록

`$issue-create` 로 넘긴다. 없으면 `issue-start` 의 `references/intake.md` 에 있는 설치 폴백 체인을 그대로 쓴다.

## 4번 — 종료

마무리 보고를 출력하고 끝낸다. 워크트리는 지우지 않는다.

```text
남은 워크트리   <n>개 — 통합하려면 $issue-merge
```

## 하지 않는 것

- **워크트리·브랜치 삭제.** 증거 URL 이 브랜치에 의존할 수 있고, 다른 작업이 진행 중일 수 있다. 정리는 `issue-merge` 가 통합을 마친 뒤에 한다.
- **`evidence/issue-<번호>` 브랜치 삭제.** 폴백으로 만들어진 증거 브랜치는 영구 보존 대상이다.
- **사용자에게 묻지 않고 다음 스킬로 넘어가기.** 이 선택은 반드시 사용자가 한다.
