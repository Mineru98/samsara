...
`issue-start` 가 이미 증거를 만들고 게시했다면 여기서는 **검사**만 한다. 처음부터 다시 만들지 않는다.
`context`의 `evidencePublished: true`는 로컬 증거와 원격 게시본이 blob 단위로 같다는 뜻이다.
`false`일 때만 누락·변경 항목을 보강하고 재게시한다.
...
`evidenceComplete` 는 `before > 0 && after > 0` 이다. 이것만으로는 부족하니 `neji-verifier` 에 아래를 확인시킨다.
...
이 경우 게시본이 바뀌므로 6~8단계의 재게시와 이슈 코멘트 갱신을 수행한다.
변경이 없고 `evidencePublished: true`면 반복하지 않는다.
