...
`/issue-start`(Codex 계열에서는 `$issue-start`) 에 이슈 번호 대신 작업 설명이 들어오는 경우를 다룬다.
...
- `--clone` 은 `$HOME/SourceCode/samsara` → `$HOME/samsara` 순으로, **부모 디렉터리가 이미 있는 첫 위치**에 clone 한다.
- `--flavor` 는 현재 `<skill>` 이 `.claude/` 밑인지 `.codex/` 밑인지로 정한다.
- 이미 로컬에 samsara 체크아웃이 있으면 clone 하지 않고 그것을 쓴다.
...
node <install-skill>/script/install-skill.js \
  https://github.com/Mineru98/samsara/tree/main/skills/issue-create --global
```

Claude와 Codex 모두 같은 루트 스킬 디렉터리를 설치한다.
...
```text
claude  https://raw.githubusercontent.com/Mineru98/samsara/refs/heads/main/skills/issue-create/SKILL.md
codex   https://raw.githubusercontent.com/Mineru98/samsara/refs/heads/main/skills/issue-create/SKILL.md
```
