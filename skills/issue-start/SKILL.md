...
<skill>
  ...
  <handoff>
    구현·증거 게시·코멘트가 끝나면 사람의 검토·승인을 기다린다.
    승인받으면 같은 워크트리에서 `issue-end` 를 실행한다.
    `issue-end` 는 게시된 증거를 재확인하고 부족하거나 바뀐 경우에만 보강한 뒤 PR 을 만든다.
  </handoff>
...
```text
skills/issue-start              # 플러그인 루트 (Claude Code / Codex)
...
```bash
sh <migrate-skill-agent>/scripts/migrate-skill-agent.sh --agent neji-verifier --target home --link --clone
```
...
`neji-verifier` 에 위임한다. 라벨, 본문 키워드, 첨부 스크린샷 유무를 근거로 삼는다.
